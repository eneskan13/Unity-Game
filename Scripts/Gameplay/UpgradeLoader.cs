...
public class UpgradeLoader : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        GetComponent<carMainController>().bulletValue = ammoValue[PlayerPrefs.GetInt("Gun" + carID.ToString())];
        FindObjectOfType<GameManager>().MaxNitro = nitroValue[PlayerPrefs.GetInt("Nitro" + carID.ToString())];
#if UNITY_EDITOR
        if (debug)
        {
